...
socket.on('updaterooms', function(rooms, current_room) {
    ...
    $.each(rooms, function(key, value) {
        if(value == current_room) {
            $('#room-list').append('<li>' + value + '</li>');
        } else {
            $('#room-list').append('<li class="availible"><a href="#" onclick="switchRoom(\'' + value + '\')">' + value + '</a></li>');
        }
    });
});
...
socket.on('newuser', function(username, data) {
    $('#message-wrap').append(username + ' ' + data + ' <br/>');
});
